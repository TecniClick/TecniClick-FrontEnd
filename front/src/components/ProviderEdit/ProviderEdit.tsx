...
const ProviderEdit = () => {
  ...
  const handleAddService = () => {
    ...
    if (matchedCategory && !services.some(service => service.id === matchedCategory.id)) {
      setServices([...services, matchedCategory]);    
      setNewService("");
      setShowSuggestions(false);
      return;
    } else if (!matchedCategory) {
      toast.warning("Solo puedes agregar una categoria pre-existente");
    } else if (services.some(service => service.id === matchedCategory.id)) {
  ...
  const submitHandler = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (token && user && data && data.title && name && data.description && data.phone && data.appointmentPrice && addressCheck()) {
